...
parser = argparse.ArgumentParser(description='Use the trained neural network to make prediction.')

parser.add_argument('--image_path', action='store',
                    default = '../aipnd-project/flowers/test/1/image_06743',
                    help='Provide path for an image.')

parser.add_argument('--save_dir', action='store',
                    dest='save_directory', default = 'checkpoint.pth',
                    help='Define location to save checkpoint in.')

parser.add_argument('--arch', action='store',
                    dest='pretrained_model', default='vgg11',
                    help='Enter pretrained model to use, the default is VGG-11.')

parser.add_argument('--top_k', action='store',
                    dest='topk', type=int, default = 3,
                    help='Enter number of top most likely classes to view, the default is 3.')
...
parser.add_argument('--gpu', action="store_true", default=False,
                    help='Turn GPU mode on or off, the default is off.')
...
print(f"This flower is potential to be a: '{names[0]}' with a probability of {round(probs[0]*100,4)}% ")